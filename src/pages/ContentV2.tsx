...
// Varied aspect ratios for a real masonry feel — hash post id to pick one
function getMasonryAspect(post: CreatorPost): string {
  if (post.type === "instagram_post") {
    // posts are 1:1 or 4:5
    const variants = ["aspect-square", "aspect-[4/5]"];
    return variants[post.id.charCodeAt(post.id.length - 1) % variants.length];
  }
  // tiktok / reels / stories — varied tall ratios
  const variants = ["aspect-[9/16]", "aspect-[3/5]", "aspect-[4/7]"];
  return variants[post.id.charCodeAt(post.id.length - 1) % variants.length];
}

function ContentCard({ post, isNew, onOpen }: ContentCardProps) {
  const aspectClass = getMasonryAspect(post);

  return (
    <button
      type="button"
      onClick={() => onOpen(post)}
      className="group mb-5 flex w-full break-inside-avoid flex-col rounded-xl border border-[var(--neutral-200)] bg-white text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[var(--brand-300)] hover:shadow-lg"
    >
